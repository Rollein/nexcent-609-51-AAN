<script setup></script>
<template>
            <section class="promo">
            <div class="container">
                <div class="row flex-column-reverse flex-lg-row">
                    <div class="col-lg-7 d-flex align-items-center">
                        <div class="promo__group">
                            <h1 style="letter-spacing: 2px;">Lessons and insights<span> from 8 years</span> </h1>
                            <p>
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit. Elementum nisi aliquet volutpat
                                pellentesque volutpat est.
                            </p>
                            <button class="button_primary-xl ">Register</button>
                        </div>
                    </div>
                    <div class="col-lg-5">
                        <img src="@/assets/images/promo-img.svg" alt="" class="d-block mx-auto ms-lg-auto me-lg-0 w-100">
                    </div>
                </div>
            </div>
        </section>
</template>

<style scoped></style>